...
export async function fetchRSCurrentUser(): Promise<ApiResponse<ReadProfile>> {
    const database = await serverSupa();
    const profiler = new ImpProfileGetter(database);

    const auth = await helpGateKeep(profiler, "access_rs_page");
...
export async function editRSCurrentUser(input: {
    name?: string;
    email?: string;
